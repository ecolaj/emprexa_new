...
export const Onboarding: React.FC<NavProps> = ({ navigate }) => {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert(t('onboarding.errorAvatar'));
    } finally {
  ...
  const handleNext = () => {
    if (step === 1) {
      if (!formData.name.trim()) {
        setErrors({ ...errors, name: t('onboarding.errorName') });
        return;
      } else {
  ...
  const renderStep1 = () => (
    <div className="max-w-xl mx-auto animate-[fade-in_0.3s_ease-out]">
      <h1 className="text-3xl font-black text-slate-900 mb-2">{t('onboarding.step1Title')}</h1>
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        <span className="text-xs font-bold text-slate-400 uppercase">
          {formData.avatar ? t('onboarding.changePhoto') : t('onboarding.yourPhoto')}
        </span>
      </div>

      <div className="space-y-6">
        <div>
          <label className="block text-sm font-bold text-slate-900 mb-1">{t('onboarding.fullName')} <span className="text-red-500">*</span></label>
          <input
            type="text"
            value={formData.name}
            onChange={(e) => {
        ...
        <div>
          <div className="flex justify-between">
            <label className="block text-sm font-bold text-slate-700 mb-1">{t('onboarding.howDoYouDefine')}</label>
            <span className="text-xs font-bold text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{t('onboarding.visibleInProfile')}</span>
          </div>
          <input
            type="text"
          ...
          />
          <p className="text-xs text-slate-400 mt-1">{t('onboarding.roleHelper')}</p>
        </div>
      </div>
    </div>
  );
  ...
  const renderStep2 = () => (
    <div className="max-w-3xl mx-auto animate-[fade-in_0.3s_ease-out]">
      <h1 className="text-3xl font-black text-slate-900 mb-2">{t('onboarding.step2Title')}</h1>
      <p className="text-slate-500 mb-8">{t('onboarding.step2Subtitle')}</p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {[
          { id: 'creator', label: t('onboarding.creator'), icon: 'lightbulb', desc: t('onboarding.creatorDesc') },
          { id: 'supporter', label: t('onboarding.supporter'), icon: 'volunteer_activism', desc: t('onboarding.supporterDesc') },
          { id: 'investor', label: t('onboarding.investor'), icon: 'savings', desc: t('onboarding.investorDesc') },
          { id: 'expert', label: t('onboarding.expert'), icon: 'psychology', desc: t('onboarding.expertDesc') },
        ].map((type) => (
          <div
            key={type.id}
  ...
  const renderStep3 = () => (
    <div className="max-w-2xl mx-auto animate-[fade-in_0.3s_ease-out]">
      <h1 className="text-3xl font-black text-slate-900 mb-2">{t('onboarding.step3Title')}</h1>
      <div className="flex flex-wrap gap-3 mt-6">
        {[t('onboarding.goalPartners'), t('onboarding.goalLearn'), t('onboarding.goalFunding'), t('onboarding.goalVolunteer'), t('onboarding.goalInspire'), t('onboarding.goalMentor')].map(goal => (
  ...
  const renderStep4 = () => (
    <div className="animate-[fade-in_0.3s_ease-out]">
      <h1 className="text-3xl font-bold text-slate-900 mb-2">{t('onboarding.step4Title')}</h1>
      <p className="text-slate-500 mb-6">{t('onboarding.step4Subtitle')}</p>
        ...
        {SDGS.map((sdg) => {
          ...
          return (
            <div key={sdg.id} onClick={() => handleSdgToggle(sdg.id)} className={`relative aspect-square rounded-xl cursor-pointer flex flex-col items-center justify-center p-2 text-center transition-all ${isSelected ? 'ring-2 ring-slate-900 scale-95 opacity-100' : 'hover:scale-105 opacity-70'}`} style={{ backgroundColor: sdg.color }}>
              ...
              <span className="text-white text-[9px] font-bold hidden sm:block">{t(`sdgs.${sdg.id}.short`) || sdg.short}</span>
            </div>
          );
        })}
      </div>
  ...
  const renderStep5 = () => (
    <div className="max-w-md mx-auto text-center animate-[fade-in_0.3s_ease-out]">
      ...
      <h1 className="text-4xl font-black text-slate-900 mb-4">{t('onboarding.step5Title')}</h1>
      <p className="text-slate-500 text-lg mb-8">{t('onboarding.step5Subtitle')}</p>
    </div>
  );
    ...
    <div className="h-screen w-full bg-background-light flex items-center justify-center p-4">
      <div className="w-full max-w-5xl bg-white rounded-2xl shadow-xl overflow-hidden flex flex-col h-[90vh]">
        <div className="shrink-0 bg-white z-10 px-8 py-6 border-b border-slate-100 flex justify-between items-center">
          <span className="text-sm font-semibold text-slate-500">{t('onboarding.step')} {step} {t('onboarding.of')} {totalSteps}</span>
        ...
        <div className="px-8 py-6 bg-slate-50 border-t border-slate-100 flex justify-between items-center shrink-0">
          <button onClick={prevStep} className={`text-slate-500 font-bold hover:text-slate-900 ${step === 1 ? 'invisible' : ''}`}>{t('onboarding.back')}</button>
          <button onClick={handleNext} className="bg-primary text-white px-8 py-3 rounded-xl font-bold hover:bg-primary-dark shadow-lg shadow-primary/20">
            {step === totalSteps ? t('onboarding.goToFeed') : t('onboarding.next')}
          </button>
        </div>
      </div>
    </div>
